# Kubernetes ABC - LAB 7

### 1) Empty Dir
> $ cd ~/7
> $ cat ~/7/empty_dir.yaml
It will create a service for the deployment and a deployment with two pods. Each pod has two containers. <br/>
The pod has defined a volume named **test** and the type is **emptyDir**.

In nginx container it is mounted under /usr/share/nginx/html and ubuntu container /busy.  <br/>
...
> $ kubectl exec -it client bash 
> # curl emptydir
You will see different messages depending on what pod the request was sent to.   <br/>
